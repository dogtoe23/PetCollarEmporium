...
function orderSummary(){
	...
	var intPet = 0;
	var intColor = 0;
	// check to see which type of pet is selected
	for (var i = 0; i < document.forms[0].rdoPet.length; i++){
		if (document.forms[0].rdoPet[i].checked){
			intPet = i;
		}
	}
	
	// set the DHTML display to include the type of pet
	switch(intPet){
		case 0:
		  strPC = strPC + '<br />Cat Collar';
//		  fltSub = 999.95;
		  break;
		case 1:
		  strPC = strPC + '<br />Dog Collar';
//		  fltSub = 1999.95;
		  break;
	}
	...
	switch(intSize){
		case 0:
		  strPC = strPC + '<br />Small ';
//		  fltSub = 999.95;
			if (intPet == 0) {
				fltSub = OptionPrice[intSize];
			} else {
				fltSub = OptionPrice[intSize+4];
			}
		  break;
		case 1:
		  strPC = strPC + '<br />Medium ';
//		  fltSub = 1999.95;
			if (intPet == 0) {
				fltSub = OptionPrice[intSize];
			} else {
...
//		  fltSub = 2999.95;
	...
	for (var i = 0; i < document.forms[0].rdoColor.length; i++){
		if (document.forms[0].rdoColor[i].checked){
			intColor = i;
		}
	}
	
	// set the DHTML display to include the Color
	switch(intColor){
		case 0:
		  strPC = strPC + '<br />Red ';
//		  fltSub = 999.95;
...
//		  fltSub = 1999.95;
		  break;
		case 2:
		  strPC = strPC + '<br />Green ';
//		  fltSub = 2999.95;
		  break;
		case 3:
		  strPC = strPC + '<br />Black ';
		  break;
	}
	
	// check to see if Diamonds were chosen
	for (var i = 0; i < document.forms[0].rdoDiamond.length; i++){
		if (document.forms[0].rdoDiamond[i].checked){
			intWarranty = i;
		}
	}
	
	// set the DHTML display to include the Diamond option
	switch(intWarranty){
		case 0:
		  strPC = strPC + "(Without Diamonds)";
		  break;
		case 1:
		  strPC = strPC + "With Diamonds";
		  fltSub += 89.95;
		  break;
	}
...
function writeCustCookie(){
	...
	SetCookie("custAddress", document.forms[0].txtStreet.value + " " + document.forms[0].txtApartment.value);
